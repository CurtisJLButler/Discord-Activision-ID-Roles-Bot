...
def post(user, id):
    log.log_message(f"Posting {id} for {user}")
    item = {"discUser": user,
            "actID": id
			}
    try: 
...
def getAll():
    log.log_message(f"getting all ActivisionID's")
    ...
    if result:
      for doc in result:
        ...
        id = doc['actID']
        # {user: id}
        actIDsArray.update({user: id})
        print("%s has Activision ID %s" % (user, id))
    return actIDsArray
...
def getID(user):
	doc = my_collection.find_one({"discUser": user})
	if doc is not None:
		id = doc['actID']
		return id
	else:
